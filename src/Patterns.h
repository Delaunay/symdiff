...
struct PatternMatcher: public StaticVisitor<PatternMatcher, bool>
{
    ...
    static bool run(Node a, Node b){
        return PatternMatcher(b).dispatch(a, b, 0);
    }

    bool dispatch(Node a, Node b, int depth, bool pleaf=false){
        ...
        if (a->id == b->id)
            return StaticVisitor<PatternMatcher, bool>::dispatch(a, b, depth, pleaf);
        return false;
    }

    bool match(Node a, Node b, int depth, bool pleaf=false){
        return dispatch(a, b, depth, pleaf);
    }
...
struct Equality: public StaticVisitor<Equality, bool>
{
    static bool run(Node a, Node b){
        return Equality().dispatch(a, b);
    }

    bool dispatch(Node a, Node b){
        if (a->id == b->id)
            return StaticVisitor<Equality, bool>::dispatch(a, b);
        return false;
    }

    bool equal(Node a, Node b){    return dispatch(a, b); }
